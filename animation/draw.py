...
class Rosto():
    ...
    def _draw_conea(self,x:int,y:int):
        cv2.circle(self.rosto, self.update_center_cornea_esquerda(x, y), 10, (255, 0, 0), cv2.FILLED)
        cv2.circle(self.rosto, self.update_center_cornea_direita(x, y), 10, (255, 0, 0), cv2.FILLED)

    def update_center_cornea_esquerda(self, x, y)->tuple:
    ...
    def update_center_cornea_direita(self, x:int, y:int)->tuple:
